...
let rec maxHelper y L =
    match L with
    | [] -> y
    | hd::tl when hd > y -> maxHelper hd tl
    | _::tl -> maxHelper y tl
...
let max L =
    ...
    | hd::tl -> maxHelper hd tl    //   TO BE IMPLEMENTED
...
let min L =
    match L with 
    | [] -> raise(System.ArgumentException("The input sequence was empty."))
    | hd::tl -> minHelper hd tl//   TO BE IMPLEMENTED    //   TO BE IMPLEMENTED
...
let rec nth L n =
    ...
    |[] -> raise(System.ArgumentException("The index is out of bounds."))
//-------------------------------------------------------------
...
let iter F L =
    iterHelper F L     //   TO BE IMPLEMENTED
...
let rec reduceHelper F x L = 
    match L with 
    |[] -> x
    |hd::tl -> reduceHelper F (F x hd) tl
...
let reduce F L =
      ...
      |hd::tl -> reduceHelper F hd tl//   TO BE IMPLEMENTED    //   TO BE IMPLEMENTED
...
let rec fold F start L =
    match L with
    |[] -> start
    |hd::tl ->  fold F (F start hd) tl  //   TO BE IMPLEMENTED
...
let  rec flatten L =
    match L with
    | [] -> []
    | hd::tl -> hd @ flatten tl     //   TO BE IMPLEMENTED
...
let rec zipHelper L1 L2 = 
    match L1 with
    |[] -> []
    |hd::tl -> match L2 with
               |[]->[]
               |hd2::tl2-> (hd,hd2) :: zipHelper tl tl2

let zip L1 L2 =
    zipHelper L1 L2     //   TO BE IMPLEMENTED
...
let rec unzipHelper L = 
    match L with
    |[] -> ([],[])
    |(x,y)::tl -> 
        let (L2, L3) = unzipHelper tl 
        (x::L2, y::L3)

let unzip L =
    unzipHelper L      //   TO BE IMPLEMENTED
...
let rec rangeHelper x stop = 
    match x with 
    |x when x = stop -> []
    |x -> x :: rangeHelper (x+1) stop
let rec range stop  =
    rangeHelper 0 stop    //   TO BE IMPLEMENTED
...
let rec range2 start stop =
    match start with 
    |start when start = stop -> []
    |start -> start :: range2 (start+1) stop   //   TO BE IMPLEMENTED
...
let rec range3 start stop step =
    match start with
    |start when start = stop -> []
    |start when stop > 0 && start > stop -> []
    |start when stop < 0 && start < stop -> []
    |start -> start :: range3 (start + step) stop step//TO BE IMPLEMENTED
...
let rec sliceHelper L start stop index = 
    match L,index with 
    |_,index when index = stop -> []
    |hd::tl, index when index >= start && index < stop -> hd::sliceHelper tl start stop (index + 1)
    |hd::tl, index when index < start -> sliceHelper L.Tail start stop (index + 1)
    

let slice L start stop =
    sliceHelper L start stop 0   //   TO BE IMPLEMENTED
...
let rec filter F L =
    ...
    |hd::tl-> filter F tl   //   TO BE IMPLEMENTED
...
let rec pairUp L =
    ...
    |[] -> []
    |_::[]->[]
    |hd::tl -> (hd,tl.Head) :: pairUp tl.Tail     //   TO BE IMPLEMENTED
...
let rec lengthsFromOrigin L =
    match L with 
    |[] -> []
    |(x,y)::tl -> distanceFromOrigin (x,y) :: lengthsFromOrigin tl   //   TO BE IMPLEMENTED
